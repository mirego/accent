defmodule Movement.Builders.TranslationUpdate do
  ...
  def build(context = %Movement.Context{assigns: %{translation: translation, text: text}, operations: operations}) do
    ...
    operation = OperationMapper.map(@action, translation, build_updated_translation(text, value_type))

    %{context | operations: Enum.concat(operations, [operation])}
  end

  defp parse_value_type(_translation, ""), do: "empty"
  defp parse_value_type(%{value_type: "null"}, value) when value != "null", do: ""
  defp parse_value_type(%{value_type: "empty"}, value) when value != "", do: ""
  defp parse_value_type(_translation, _value), do: nil

  defp build_updated_translation(text, nil), do: %{text: text}

  defp build_updated_translation(text, value_type) do
    %{text: text, value_type: value_type}
  end
end
