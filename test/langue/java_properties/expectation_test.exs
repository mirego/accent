defmodule LangueTest.Formatter.JavaProperties.Expectation do
  ...
  defmodule PlaceholderValues do
    ...
    def entries do
      [
        %Entry{index: 1, key: "single", value: "${const:java.awt.event.KeyEvent.VK_CANCEL}", placeholders: ~w(${const:java.awt.event.KeyEvent.VK_CANCEL}), value_type: "string"},
        %Entry{index: 2, key: "multiple", value: "${single}, ${sys:user.home}/settings.xml", placeholders: ~w(${single} ${sys:user.home}), value_type: "string"},
        %Entry{index: 3, key: "duplicate", value: "${env:JAVA_HOME}, ${env:JAVA_HOME}!", placeholders: ~w(${env:JAVA_HOME} ${env:JAVA_HOME}), value_type: "string"},
