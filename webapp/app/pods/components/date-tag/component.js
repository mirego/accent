...
export default Component.extend({
  ...
  formattedDatetime: computed('date', function() {
    const format = this.i18n.t('components.date_tag.formatted_date_time_format').toString();
  ...
  humanizedDate: computed('date', function() {
    const format = this.i18n.t('components.date_tag.humanized_date_title_format').toString();
    return dateFormat(new Date(this.date), format);
  })
});
